...
for (subj in unique(sim_data$subject)) {
  ...
  simulated_responses <- simulate_responses(data = sim_data_subj, agent = "equal weights", w = "equal", c = 3)
...
for (subj in names(model_fits)) {
  cat("\n--- Summary for Subject", subj, "---\n")
  print(model_fits[[subj]]$summary())
}
...
# Print and save diagnostics per subject
for (subj in names(model_fits)) {
  cat("\n--- Summary for Subject", subj, "---\n")
...
# Plot traceplots
for (subj in names(model_fits)) {
  cat("Generating traceplots for subject:", subj, "\n")
...
subj_1 <- subject_1_diagnostics %>% 
  ...
  select(variable, mean, median, sd, mad)
...
for (subj in names(model_fits_emp)) {
  cat("\n--- Summary for Subject", subj, "---\n")
  print(model_fits_emp[[subj]]$summary())
}
...
# for (subj in names(model_fits_emp)) {
#   cat("\n--- Diagnostics for Subject", subj, "---\n")
#   print(model_fits_emp[[subj]]$cmdstan_diagnose())
# }
